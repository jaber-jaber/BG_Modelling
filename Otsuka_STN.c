...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 13, "area");
 	...
 	ivoc_help("help ?1 stn Otsuka_STN.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) {int _reset = 0; {
   ...
   Dcai = 337.1 * ( - ica ) / ( 2.0 * FARADAY ) - .2 * cai ;
   Da = ( a_inf - a ) / tau_a ;
   Db = ( b_inf - b ) / tau_b ;
   Dr = ( r_inf - r ) / tau_r ;
   }
 ...
 static int states (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) { {
   ...
   evaluate_fct2 ( _threadargscomma_ cai ) ;
    ...
    cai = cai + (1. - exp(dt*(( - ( .2 )*( 1.0 ) ))))*(- ( ( ( 337.1 )*( ( - ica ) ) ) / ( 2.0 * FARADAY ) ) / ( ( - ( .2 )*( 1.0 ) ) ) - cai) ;
...
#if NMODL_TEXT
static const char* nmodl_filename = "Otsuka_STN.mod";
static const char* nmodl_file_text = 
